 ...
 * 
 * @param {string} name - The name of the icon to render (Lucide icon name or custom icon name)
 * @param {number} size - The size of the icon (default: 24)
 * @param {string} color - The color of the icon (default: 'black')
 * @param {object} style - Additional styles to apply to the icon container
 */
const CustomIcon = ({ 
  name, 
  size = 24, 
  ...
  if (customIcons[name]) {
    const { viewBox, path } = customIcons[name];
